...
# ----------------------------------------
    variable "aws_region_list" {
      description = "list of the AWS regions for the crdb cluster - For single cluster this should be one item"
      type = list
      default = ["us-east-1",]
    }

    variable "aws_instance_keys" {
      description = "The instance keys - they should match the regions and the order in the aws_region_list.  These will be used when the ec2 instances are created in each region.  For a single cluster this shoudl be one item."
      type = list
      default = ["nollen-cockroach-us-east-1-kp01",]
    }
...
# ----------------------------------------
    variable "vpc_cidr_list" {
      description = "The CIDR for the VPC.  For a single region cluster this should have just one element."
      type = list
...
# ----------------------------------------
    ...
    variable "run_init" {
      description = "'yes' or 'no' to run the cockroachDB init command."
      type        = string
      default     = "yes"
      validation {
    ...
    variable "create_admin_user" {
      description = "'yes' or 'no' to include an admin user in the database with associated cert credentials."
      type        = string
      default     = "yes"
      validation {
